...
class TefnutController:
    state = {
        "temp time": time.time() - DELAY_TEMP,
    ...
    def controler_loop(self):
        try:
            ...
            delay = (
                ON_DELAY_HUMIDITY
                if self.state["state"] == STATE.ON
                else OFF_DELAY_HUMIDITY
            )
            ...
            if self.state["temp delay"] >= DELAY_TEMP:
                ...
                (
                    current_values["current_temp"],
                    current_values["future_temp"],
                    current_values["target_temp"],
                    current_values["outdoor_humidity"],
                ) = get_temperature()
    ...
    def humidifier_controller(self):
        ...
        if (
            self.state["temp delay"] >= TEMP_EMERGENCY_DELAY
            and self.state["mode"] == MODE.AUTO
        ):
            ...
            self.state["target_humidity"] = settings.get(
                "GENERAL.emergency_target", default=30
            )
            output = -5

        if self.state["mode"] == MODE.MANUAL:
            self.state["target_humidity"] = settings.get(
                "GENERAL.manual_target", default=40
            )
            logger.debug("Manual")
        ...
        if (
            self.state["humidity"] < self.state["target_humidity"] - deadband
            and self.state["state"] != STATE.ON
        ):
            ...
            self.state["humidity"] > self.state["target_humidity"] + deadband
            and self.state["state"] != STATE.OFF
        ):
            self.humidifier.turn_off()
            logger.info("Stopping Humidifier")
            self.state["state"] = STATE.OFF
    ...
    def data_collection_logic(self, current_values):
        ...
        if all(
            [
                "current_temp" in current_values,
            ...
            if all(
                ...
                point = Point("humidity").field(
                    "outdoor", float(current_values["outdoor_humidity"])
                )
                influx_client.write(point)

                if (
                    "temp time" in current_values
                    and current_values["temp time"] is not None
                ):
        ...
        point = (
            Point("humidity")
            .field("target", float(self.state["target_humidity"]))
        ...
        point = (
            Point("hardware")
            .field("cpu", float(psutil.cpu_percent()))
