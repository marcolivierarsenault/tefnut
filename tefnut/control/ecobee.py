...
class ecobee:
    def __init__(self, pyecobee_db_path):
        ...
        if (
            self.ecobee_service.access_token is None
            and self.ecobee_service.authorization_token is not None
        ):
            self.request_tokens()

    def is_active(self):
        return (
            self.ecobee_service.authorization_token is not None
            and self.ecobee_service.access_token is not None
            and datetime.now(pytz.utc) <= self.ecobee_service.access_token_expires_on
        )
    ...
    def request_tokens(self):
        try:
            ...
            logger.warning(
                "Please go congigure your PIN if not done: %s", self.get_pin()
            )
            return -1
    ...
    def authorize(self):
        try:
            ...
            self.persist_to_shelf()
            logger.warning(
                "Please go congigure your PIN if not done: %s",
